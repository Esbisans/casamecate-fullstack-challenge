...
function AnimatedBar({
  ...
  // Trigger animation after stagger delay. Double-rAF before the timer
  // guarantees the initial render with width/height=0 paints first,
  // so the CSS transition has a starting frame even when delay=0
  // (single-bar charts).
  useEffect(() => {
    let raf1 = 0;
    let raf2 = 0;
    ...
    const startMs = index * staggerDelay * 1000;

    raf1 = requestAnimationFrame(() => {
      raf2 = requestAnimationFrame(() => {
        if (startMs > 0) {
          timeout = setTimeout(() => setIsAnimated(true), startMs);
        } else {
          setIsAnimated(true);
        }
      });
  ...
  const getFadeOpacity = () => {
    if (isFaded) return fadedOpacity;
  ...
  // "grow" animation — canonical bklit pattern: width/height as SVG
  // attributes, CSS transition declared in style. SVG2 makes geometry
  // properties CSS-animatable, so attribute changes drive the transition.
