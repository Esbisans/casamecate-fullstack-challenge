...
export function Line({
  ...
  const segmentBounds = useMemo(() => {
    ...
    // Special case: with only 2 data points, highlight only the half of the line
    // closest to the hovered endpoint (otherwise both halves cover the whole line)
    if (data.length === 2) {
      const first = data[0];
      const last = data[1];
      if (!(first && last)) {
      ...
      const firstX = xScale(xAccessor(first)) ?? 0;
      const lastX = xScale(xAccessor(last)) ?? 0;
      const midX = (firstX + lastX) / 2;
      const [a, b] = idx === 0 ? [firstX, midX] : [midX, lastX];
      const startLength = findLengthAtX(a);
      const endLength = findLengthAtX(b);
      return {
        startLength,
        segmentLength: endLength - startLength,
        isActive: true,
  ...
  return (
    <>
      {/* Gradient definition for fading edges
          Uses userSpaceOnUse so it works on flat horizontal lines (zero-height bbox) */}
      {fadeEdges && (
        <defs>
          <linearGradient
            id={gradientId}
